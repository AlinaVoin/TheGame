...
class GameState {
    ...
    public boolean canMove(int x, int y) { //может ли фишка походить
        ...
        if (A[y][x] == white) {
            ...
            if (y < N - 2 && x > 1 && (A[y + 1][x - 1] == black || A[y + 1][x - 1] == BK) && A[y + 2][x - 2] == 0)
                return true;
            if (y < N - 2 && x < N - 2 && (A[y + 1][x + 1] == black || A[y + 1][x + 1] == BK) && A[y + 2][x + 2] == 0)
                return true;
            if (y > 1 && x > 1 && (A[y - 1][x - 1] == black || A[y - 1][x - 1] == BK) && A[y - 2][x - 2] == 0)
                return true;
            if (y > 1 && x < N - 2 && (A[y - 1][x + 1] == black || A[y - 1][x + 1] == BK) && A[y - 2][x + 2] == 0)
                return true;
        } else if (A[y][x] == black) {
            if (y > 0 && x > 0 && A[y - 1][x - 1] == 0) return true;
            if (y > 0 && x < N - 1 && A[y - 1][x + 1] == 0) return true;

            if (y < N - 2 && x > 1 && (A[y + 1][x - 1] == white || A[y + 1][x - 1] == WK) && A[y + 2][x - 2] == 0)
                return true;
            if (y < N - 2 && x < N - 2 && (A[y + 1][x + 1] == white || A[y + 1][x + 1] == WK) && A[y + 2][x + 2] == 0)
                return true;
            if (y > 1 && x > 1 && (A[y - 1][x - 1] == white || A[y - 1][x - 1] == WK) && A[y - 2][x - 2] == 0)
                return true;
            if (y > 1 && x < N - 2 && (A[y - 1][x + 1] == white || A[y - 1][x + 1] == WK) && A[y - 2][x + 2] == 0)
                return true;
        } else if (A[y][x] == WK) {
            if (y < N - 1 && x > 0 && A[y + 1][x - 1] == 0) return true;
            if (y < N - 1 && x < N - 1 && A[y + 1][x + 1] == 0) return true;
    ...
    public void move(int horCor, int verCor) {
        int[] deadCell = {-1, -1};
        if (currentMove == 1) {
            ...
            if (A[N - 1 - verCor][horCor] == black || A[N - 1 - verCor][horCor] == BK) {
                ...
                if (isSelected && checkMove(selectedX, selectedY, horCor, N - 1 - verCor, deadCell)) {
                    ...
                    if (deadCell[0] >= 0) {
                        ...
                        movesFromLastEat++;
                        if (movesFromLastEat >= 2 * MOVE_LIMIT_WITHOUT_EAT) {
                            gameMSG = "Ничья!";
                            gameOn = false;
                        }
                        currentMove = 1;
    ...
    public void start(Stage primaryStage) {
        ...
        EventHandler<MouseEvent> clickHandler = new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent mouseEvent) {
                if (!X.gameOn) {
                    X.gameMSG = "Текущая игра закончена. Чтобы начать новую игру нажмите <Enter>";
                    repaintBoard();
                    return;
                }
                ...
                if (x <= 0 || y <= 0) return;
                    ...
                    int horCor = x / D;
                    int verCor = y / D;
                    if (horCor >= X.N || verCor >= X.N) return;
                    if ((horCor + verCor) % 2 == 0) return;
                    X.move(horCor, verCor);
                    repaintBoard();
                }
            }
        };
        ...
        scene.setOnKeyPressed(e -> {
            if (e.getCode() == KeyCode.ENTER) {
                if (!X.gameOn) {
                    X = new GameState();
                    X.gameMSG = "Новая игра! Ход белых.";
                    repaintBoard();
                }
            }
        });
    ...
    public void repaintBoard() {
        ...
        for (int i = 0; i < X.N; i++) {
            for (int j = 0; j < X.N; j++) {
                ...
                gc.fillRect(W / 2 - (X.N / 2) * D + j * D, W / 2 + (X.N / 2) * D - i * D - D, D, D);
            }
        }
        gc.setFill(Color.WHITE);
        for (int i = 0; i < X.N; i++) {
            gc.fillText(String.valueOf((char) ('0' + i + 1)), 30, W / 2 + (X.N / 2) * D - i * D - D + D / 2 + 5);
        }
        for (int i = 0; i < X.N; i++) {
            gc.fillText(String.valueOf((char) ('A' + i)), W / 2 - (X.N / 2) * D + i * D + D / 2 - 5, 60);
        }

        for (int i = 0; i < X.N; i++) {
            for (int j = 0; j < X.N; j++) {
                int weight = W / 2 - (X.N / 2) * D + j * D;
                int length = W / 2 + (X.N / 2) * D - i * D - D;
                if (X.A[i][j] == X.black) {
                    gc.drawImage(darkCircle, weight, length);
                } else if (X.A[i][j] == X.white) {
                    gc.drawImage(lightCircle, weight, length);
                } else if (X.A[i][j] == X.WK) {
                    gc.drawImage(lightKing, weight, length);
                } else if (X.A[i][j] == X.BK) {
            ...
            if (X.isSelected) {
                ...
                gc.strokeRect(W / 2 - (X.N / 2) * D + X.selectedX * D, W / 2 + (X.N / 2) * D - X.selectedY * D - D, D, D);
            }
        }
        if (X.gameMSG.length() > 0) {
            Color msgColor = Color.color(1, 0, 0);
            gc.setFill(msgColor);
            gc.setFont(new Font("Comic Sans MS", 18.0));
            gc.fillText(X.gameMSG, 20, W - 20);
            X.gameMSG = "";
        }
    }
}